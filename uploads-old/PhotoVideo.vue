...
video {
    ...
    height: auto;
    object-fit: contain;
    /* Additional iOS Safari control hiding */
    -webkit-media-controls: none !important;
    -webkit-media-controls-panel: none !important;
    -webkit-video-controls: none !important;
    -webkit-full-screen-media-controls-panel: none !important;
}
...
/* Video styles - Hide all native video controls */
...
/* iOS Safari specific - Hide picture-in-picture button */
video::-webkit-media-controls-picture-in-picture-button {
    display: none !important;
}

/* iOS Safari specific - Hide casting button */
video::-webkit-media-controls-wireless-playback-picker-button {
    display: none !important;
}

/* iOS Safari specific - Hide rewind and fast-forward buttons */
...
/* Hide the overlay play button on iOS */
video::-webkit-media-controls-overlay-play-button {
    display: none !important;
}

/* Hide the start playback button */
...
watch(
  ...
  async (newVideos) => {
    ...
    console.log("💡 videos updated, refs now:", videoRefs.value.map(v => v?.tagName));

    newVideos.forEach((videoData, index) => {
      const videoEl = videoRefs.value[index];
      if (!videoEl || !videoData?.src) return;
      ...
      if (Hls.isSupported()) {
        ...
        hls.on(Hls.Events.MANIFEST_PARSED, () => {
          if (!isMobile.value && index === getInitialSlideIndex()) {
...
onMounted(async () => {
   ...
   await nextTick();
     ...
     if (videoRefs.value && videoRefs.value.length > 0) {
        ...
        videoRefs.value.forEach((video, index) => {
        const url = props.videos[index]

        if (!video || !url.src) return
        
        if (Hls.isSupported()) {
        const hls = new Hls()
        hls.loadSource(url?.src)
